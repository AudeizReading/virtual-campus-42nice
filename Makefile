...
launch:
	@docker build -t ${NAME}:on-built ./ && \
		docker run -it --name=${NAME} --env="DISPLAY=${DISPLAY}:0" --env="/.Xauthority" --platform=linux/amd64 --net=host -v /tmp/.X11-unix:/tmp/.X11-unix -v ~/.Xauthority:/.Xauthority ${OPTSINSTALL} ${NAME}:on-built
